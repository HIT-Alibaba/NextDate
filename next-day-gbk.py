...
class LunarSolarConverter:
    lunar_month_days = [1887, 0x1694, 0x16aa, 0x4ad5, 0xab6, 0xc4b7, 0x4ae, 0xa56, 0xb52a,0x1d2a,
    ...
    solar_1_1 = [1887,    0xec04c, 0xec23f, 0xec435, 0xec649, 0xec83e, 0xeca51, 0xecc46, 0xece3a, 0xed04d,
...
def exchange(year,mon,day):
    ...
    solar = Solar(int(year),int(mon),int(day))
    lunar = converter.SolarToLunar(solar)

    year   = int(vars(lunar)['lunarYear'])
    x = year
    year_1 = year/1000
    year   = year%1000
    year_2 = year/100
    ...
    lunarYear = cYear[year_1] + cYear[year_2] + cYear[year_3] + cYear[year] + "年"

    if vars(lunar)['isleap']==True:
        lunarMonth = "润" + cMonName[int(vars(lunar)['lunarMonth'])] + "月"
    else:
        lunarMonth = cMonName[int(vars(lunar)['lunarMonth'])] + "月"

    lunarDay = cDayName[int(vars(lunar)['lunarDay'])]
    
    wCurYear = vars(solar)['solarYear']
    szShuXiang = cShuXiang[(((int(x)-4) % 60) % 12)]
    zNongli = szShuXiang + '(' + cTianGan[(((wCurYear-4) % 60) % 10)] + cDiZhi[(((wCurYear - 4) % 60) % 12)] + ')年'

    print "农历："
    print zNongli
    print lunarYear + lunarMonth + lunarDay

def is_valid_date(y,m,d):
    year = int(y)
    mon  = int(m)
    day  = int(d)
    day_isleap = [0,31,29,31,30,31,30,31,31,30,31,30,31]
    day_notleap = [0,31,28,31,30,31,30,31,31,30,31,30,31]
    ...
    if 0<mon and mon<=12:
        ...
        return False
    if year%400==0:
        if 0<day and day<=day_isleap[mon]:
            return True
        else:
            return False
    elif year%100==0 and year%400!=0:
        if 0<day and day<=day_notleap[mon]:
            return True
        else:
            return False
...
def weekday(year,mon,day):
    import time
    import datetime
    anyday=datetime.datetime(year,mon,day).strftime("%w")
    weekday = ["天","一","二","三","四","五","六"]
    print "星期"+weekday[int(anyday)]
if __name__ == '__main__':
    while(1):
        year = int(raw_input("请输入年："))
        mon  = int(raw_input("请输入月："))
        day  = int(raw_input("请输入日："))
        if(is_valid_date(year,mon,day)==True):
            if mon==1 and day==31:
                mon = 2
                day = 1
                ...
                mon = 3
                day = 1
            elif mon==3 and day==31:
                mon = 4
                day = 1
            elif mon==4 and day==30:
                mon = 5
                day = 1
            elif mon==5 and day==31:
                mon = 6
                day = 1
            elif mon==6 and day==30:
                mon = 7
                day = 1
            elif mon==7 and day==31:
                mon = 8
                day = 1
            elif mon==8 and day==31:
                mon = 9
                day = 1
            elif mon==9 and day==30:
                mon = 10
                day = 1
            elif mon==10 and day==31:
                mon = 11
                day = 1
            elif mon==11 and day==30:
                mon = 12
                day = 1
                ...
                day += 1
            print "The next day :"
            print str(year)+"年"+str(mon)+"月"+str(day)+"日"
            weekday(year,mon,day)
            exchange(year,mon,day)
            print "--------------------------------------------------------------------------------"
        else:
            print "日期错误"
